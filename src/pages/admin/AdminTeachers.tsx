...
import { format } from "date-fns"; // Using date-fns directly
...
const AdminTeachers = () => {
  ...
  const exportTeacherData = (teacher: Teacher, fileType: string) => {
    try {
      if (fileType === 'csv') {
        ...
        toast.info(`Export as PDF is not implemented yet`);
      } else if (fileType === 'doc') {
        toast.info(`Export as DOC is not implemented yet`);
      } else {
        toast.info(`Export as ${fileType.toUpperCase()} is not implemented yet`);
      }
    } catch (error) {
  ...
  const renderFieldValue = (value: any, isLink = false) => {
    ...
    if (isLink && value.toString().startsWith('http')) {
      return (
        <a 
          href={value} 
          target="_blank" 
        ...
        >
          {value.toString().substring(0, 30)}... <ExternalLink className="h-3 w-3 ml-1" />
        </a>
      );
    }
                        ...
                        <div>
                          <div className="flex justify-between mb-4">
                            ...
                            <div className="flex gap-2">
                              ...
                              <Button variant="outline" size="sm" onClick={() => exportTeacherData(teacher, 'pdf')}>
                                <Download className="h-4 w-4 mr-1" /> Export PDF
                              </Button>
                              <Button variant="outline" size="sm" onClick={() => exportTeacherData(teacher, 'doc')}>
                                <Download className="h-4 w-4 mr-1" /> Export DOC
                              </Button>
                            </div>
                          </div>
                          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 text-sm mb-6">
                            ...
                            <div>
                              ...
                              <p>{teacher.date_of_joining ? format(new Date(teacher.date_of_joining), 'PP') : 'N/A'}</p>
                            </div>
                            {teacher.address && (
                              <div>
                            ...
                            {teacher.cabin_no && (
                              <div>
                                <p className="font-medium">Cabin No:</p>
                                <p>{teacher.cabin_no}</p>
                              </div>
                            )}
                            {teacher.block && (
                              <div>
                                  ...
                                  <div className="flex justify-between items-start mb-2">
                                    <div>
                                      ...
                                      <p className="text-sm text-gray-600">
                                        {achievement.achievement_type} | {achievement.date_achieved ? format(new Date(achievement.date_achieved), 'PP') : 'N/A'}
                                      </p>
                                    </div>
